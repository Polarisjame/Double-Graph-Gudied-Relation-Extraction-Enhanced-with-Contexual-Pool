...
from models.GAIN_new import GAIN_GloVe, GAIN_BERT
    ...
    for epoch in range(start_epoch, opt.epoch + 1):
        ...
        for ii, d in enumerate(train_loader):
            ...
            if global_step % log_step == 0:
                ...
                logging(
                    '| epoch {:2d} | step {:4d} |  ms/b {:5.2f} | lr {:.3E} |train loss {:5.3f} | NA acc: {:4.2f} | not NA acc: {:4.2f}  | tot acc: {:4.2f} '.format(
                        epoch, global_step, elapsed * 1000 / log_step, optimizer.param_groups[1]['lr'], cur_loss * 1000,
                        acc_NA.get(), acc_not_NA.get(),
                        acc_total.get()))
                total_loss = 0
                start_time = time.time()
