...
from models.GAIN_new import Bert
...
class DGLREDataset(IterableDataset):
    ...
    def __init__(self, src_file, save_file, word2id, ner2id, rel2id,
        ...
        if os.path.exists(save_file):
            ...
            for i, doc in enumerate(ori_data):
                ...
                # entity_list:entity1:[{mention1},{mention2}...], entity2:[{},{}...] ...

                Ls = [0]
                L = 0

                # entity_list中sent_id转为int，位置改为该段文字中的绝对位置global_pos
                for x in sentences:
                    L += len(x)
                    Ls.append(L)
                ...
                # generate positive examples（将有关系的实体三元组（head,tail,rel）保存在self.instance_in_train
                train_triple = []  # [(idx of head,idx of tail), (idx of head,idx of tail), (idx of head,idx of tail)]
                new_labels = []
                for label in labels:
                ...
                word_id = np.zeros((self.document_max_length,), dtype=np.int32)  # words转为id
                pos_id = np.zeros((self.document_max_length,), dtype=np.int32)  # 实体出现位置标记为该实体id
                ner_id = np.zeros((self.document_max_length,), dtype=np.int32)  # 实体出现位置标记为该实体ner的id
                mention_id = np.zeros((self.document_max_length,), dtype=np.int32)  # 实体出现位置标记为具体提及的id

                for iii, w in enumerate(words):
                    word = word2id.get(w.lower(), word2id['UNK'])
                    word_id[iii] = word

                entity2mention = defaultdict(list)  # {实体id:[提及id,提及id...]}
                mention_idx = 1
                ...
                entity_graph, path = self.create_entity_graph(Ls, pos_id)
    ...
    def create_graph(self, Ls, mention_id, entity_id, entity2mention):
        ...
        :param Ls: [每个句子的开始位置：0,L0,L0+L1...]
        :param mention_id: ndarray ,实体出现位置标记为具体提及的id
        :param entity_id: ndarray ,实体出现位置标记为该实体id
        :param entity2mention: dict ,{实体id:[提及id,提及id...]}

        :return: graph: dgl.graph
        """

        d = defaultdict(list)  # {"('node', 'intra', 'node')":[], }
        ...
        if not d[('node', 'intra', 'node')]:
            d[('node', 'intra', 'node')].append((entity2mention[1][0], 0))  # 若为空，连接第一个提及的节点到文档节点

        for i in range(1, len(Ls)):
            tmp = dict()  # {mention_id:entity_id}
            for j in range(Ls[i - 1], Ls[i]):  # 遍历第i-1个句子
                if mention_id[j] != 0:
                    tmp[mention_id[j]] = entity_id[j]
            mention_entity_info = [(k, v) for k, v in tmp.items()]

            # add self-loop & to-global-node edges
            for m in range(len(mention_entity_info)):
                # self-loop
                # d[('node', 'loop', 'node')].append((mention_entity_info[m][0], mention_entity_info[m][0]))

                # to global node
                d[('node', 'global', 'node')].append((mention_entity_info[m][0], 0))  # 每个提及节点都与文档节点连接
    ...
    def create_entity_graph(self, Ls, entity_id):
        """
        创建实体级别图

        :param Ls: [每个句子的开始位置：0,L0,L0+L1...]
        :param entity_id: 实体出现位置标记为该实体id
        ...
        d = defaultdict(set)  # 所有出现在同一句话的实体相连

        for i in range(1, len(Ls)):
            tmp = set()  # 该句子实体id的集合(1,2,3,...)
            ...
            for i, doc in enumerate(ori_data):
                ...
                for j in range(len(entity_list)):
                    for k in range(len(entity_list[j])):
                        ...
                        dl = Ls[sent_id]
                        pos0, pos1 = entity_list[j][k]['pos']
                        entity_list[j][k]['global_pos_neural'] = (pos0 + dl, pos1 + dl)
                
# ---------------------------------------------------------------------------------------------------------
# 在文本的entity前后插入special token '*'，并更新插入后的数据集信息如每个单词的位置等
# ---------------------------------------------------------------------------------------------------------

                mention_idx = 1
                skip_ids = defaultdict()
                sentences_BO = [[0 for j in range(len(sentences[ind]))] for ind in range(len(sentences))]
                special_token_sentences = [[] for ind in range(len(sentences))]
                for j in range(len(entity_list)):
                    for k in range(len(entity_list[j])):
                        sent_id = int(entity_list[j][k]['sent_id'])
                        pos0, pos1 = entity_list[j][k]['pos']
                        if sum(sentences_BO[sent_id][pos0:pos1]) > 0:
                            skip_ids[mention_idx] = list(set(sentences_BO[sent_id][pos0:pos1]))[0]
                            mention_idx += 1
                            continue
                        sentences_BO[sent_id][pos0:pos1] = [mention_idx for ind in range(pos0, pos1)]
                        mention_idx += 1

                label_0 = 0
                special_token_sentences_word = [[] for ind in range(len(sentences))]
                for ind in range(len(sentences)):
                    for word, label in zip(sentences[ind], sentences_BO[ind]):
                        if label != 0:
                            if label_0 == 0:
                                special_token_sentences[ind].append('*')
                                special_token_sentences[ind].append(label)
                                special_token_sentences_word[ind].append('*')
                                special_token_sentences_word[ind].append(word)
                            else:
                                special_token_sentences[ind].append(label)
                                special_token_sentences_word[ind].append(word)
                            label_0 = label
                ...
                L = 0
                mention_idx = 1
                for x in special_token_sentences:
                    L += len(x)
                    Ls.append(L)
                for j in range(len(entity_list)):
                    for k in range(len(entity_list[j])):
                        ...
                        dl = Ls[sent_id]
                        sent = special_token_sentences[sent_id]
                        sent_re = sent.copy()
                        sent_re.reverse()
                        if mention_idx in skip_ids.keys():
                            pos0 = sent.index(skip_ids[mention_idx])
                            pos1 = len(sent_re) - sent_re.index(skip_ids[mention_idx])
                        else:
                            pos0 = sent.index(mention_idx)
                            pos1 = len(sent_re) - sent_re.index(mention_idx)
                        entity_list[j][k]['global_pos'] = (pos0 + dl, pos1 + dl)
                        entity_list[j][k]['token_pos'] = pos0 + dl - 1
                        mention_idx += 1
                ...
                # generate document ids
                words = []
                for sentence in special_token_sentences_word:
                    for word in sentence:
                        words.append(word)
                ...
                pos_id = np.zeros((self.document_max_length,), dtype=np.int32)  # 实体出现位置标记为该实体id
                ner_id = np.zeros((self.document_max_length,), dtype=np.int32)  # 实体出现位置标记为该实体ner的id
                mention_id = np.zeros((self.document_max_length,), dtype=np.int32)  # 实体出现位置标记为具体提及的
                token_id = np.zeros((self.document_max_length,), dtype=np.int32)  # 实体出现位置标记为具体提及的
                ...
                for idx, vertex in enumerate(entity_list, 1):
                    vex = vertex
                    for v in vertex:
                        sent_id, (pos0, pos1), ner_type, token_pos = v['sent_id'], v['global_pos'], v['type'], v['token_pos']
                        ...
                        ner_id[pos0:pos1] = ner2id[ner_type]
                        mention_id[pos0:pos1] = mention_idx
                        token_id[token_pos] = mention_idx
                        entity2mention[idx].append(mention_idx)
                        mention_idx += 1
                        already_exist.add((pos0, pos1))
...
class DGLREDataloader(DataLoader):
    ...
    def __iter__(self):
        ...
        context_token_ids = torch.LongTensor(self.batch_size, self.max_length).cpu()
        ...
        for idx, minibatch in enumerate(self.batches):
            ...
            for mapping in [context_word_ids, context_token_ids, context_pos_ids, context_ner_ids, context_mention_ids,
            ...
            for i, example in enumerate(minibatch):
                title, entities, labels, na_triple, word_id, token_id, pos_id, ner_id, mention_id, entity2mention, graph, entity_graph, path = \
                    example['title'], example['entities'], example['labels'], example['na_triple'], \
                    example['word_id'], example['token_id'], example['pos_id'], example['ner_id'], example['mention_id'], example[
                ...
                context_token_ids[i, :word_num].copy_(torch.from_numpy(token_id))
                ...
                if self.dataset_type == 'train':
                    ...
                    for j, (h_idx, t_idx) in enumerate(train_tripe):
                        ...
                        delta_dis = hlist[0]['global_pos_neural'][0] - tlist[0]['global_pos_neural'][0]
                        if delta_dis < 0:
            ...
            yield {'context_idxs': get_cuda(context_word_ids[:cur_bsz, :batch_max_length].contiguous()),
                   'context_tokens': get_cuda(context_token_ids[:cur_bsz, :batch_max_length].contiguous()),
