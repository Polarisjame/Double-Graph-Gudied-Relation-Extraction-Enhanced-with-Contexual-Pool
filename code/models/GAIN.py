...
class GAIN_BERT(nn.Module):
    ...
    def forward(self, **params):
        ...
        # ---------------------------------------------------------------------------------------------------------
        # 上一行利用BERT分别得到了每个单词的编码 encoder_outputs 以及 文档整体编码 sentence_cls
        # 346-365行的循环将BERT提取的mention特征平均后赋值给每个图中对应的mention节点， 这里需要将mention特征改为前面special token的特征
        # 同时还需要利用注意力机制嵌入mention的上下文信息
        # ---------------------------------------------------------------------------------------------------------
        ...
        graph_big = dgl.batch_hetero(graphs)
        output_features = [features]  # 收集每层GCN的特征
        ...
        # ---------------------------------------------------------------------------------------------------------
        # 下面一直到408行实现了mention到entity特征的转换并赋值给了实体图中的节点
        # 需要将转换方式改成Transformer
        # ---------------------------------------------------------------------------------------------------------
